# Plano de Ensino

1 INTRODUÇÃO  
1.1 Percepção Visual e Formação de Imagens  
1.2 Sensores e Aquisição de Imagens  
1.3 Amostragem e Quantização  
1.4 Espaço de cores. 

2 PRÉ-PROCESSAMENTO  
2.1 Filtragem: Domínio espaço e frequência  
2.2 Morfologia matemática  

3 DETECÇÃO E SEGMENTAÇÃO  
3.1 Detecção de descontinuidades e bordas  
3.2 Limiarização (global e Local)  
3.3 Segmentação orientada a regiões  

4 EXTRAÇÃO DE CARACTERÍSTICAS  
4.1 Esquemas de representação  
4.2 Propriedades geométricas  
4.3 Representação de Formas  
4.4 Descritores  

5 ANÁLISE E CLASSIFICAÇÃO  
5.1 Elementos de análise de imagens  
5.2 Padrões e classes de padrões  
5.3 Métodos de classificação  

6. REDES NEURAIS CONVOLUCIONAIS  
6.1 Introdução às Redes Neurais Convolucionais  
6.2 Fundamentos de Redes Neurais  
6.3 Convolução  
6.4 Camadas de Convolução  
6.5 Pooling  
6.6 Camadas de Normalização  
6.7 Camadas de Ativação  
6.8 Arquiteturas de CNN  
6.9 Treinamento de Redes Neurais Convolucionais  
6.10 Transfer Learning  

7. COMPRESSÃO DE IMAGENS  
7.1 Sistemas de Codificação e Compressão  
7.2 Padrões de Compressão de Imagens  
